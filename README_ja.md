...
多要素認証(2FA)サービス用のシンプルなCUIベースのTOTPシークレットマネージャ.
...
- シークレットの削除と名前変更
- ローカル MCP サーバとして起動すると、Agentツールを使用して操作可能
...
モジュールパッケージをpythonのパッケージとしてインストールできます.  
...
```json
{
  ...
  // 詳細な手順はそれぞれ使用するAgentツールののマニュアルを参照してください.
  "mcpServers" {
    "mktotp": {
      "type": "stdio",
      "command": "uv",
